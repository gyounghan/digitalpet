...
## 3.7 설화 영물(히든 종) 레퍼런스 아트 — 마커 기반

설화 영물 6종(samjoko/gumiho/moonrabbit/haetae/dokkaebi/hwangryong)은
`tool/dump_reference_art.py`가 **마커 레퍼런스**(`tool/refs/{종}_눈입.png` —
APK 번들 제외를 위해 assets/ 밖)에서 3스테이지를 분리해
`tool/hidden_species_art.py`로 굽는다. 마커 레퍼런스는 사용자가
**눈=마젠타(#FF00FF), 입=시안(#00FFFF)** 점을 직접 찍은 스테이지별
픽셀아트로, 실루엣과 눈/입 좌표가 같은 이미지에서 나오므로 자동 눈 검출의
오검출·잔상 문제가 원천적으로 없다:

- **눈/입 좌표**: 마커 픽셀 클러스터를 그리드로 축소 매핑해 rect/점 자동 산출
  (스테이지당 눈 1~2개). 수동 `EYE_OVERRIDE`/`MOUTH_OVERRIDE`는 마커 실패
  시의 폴백으로만 남아 있다.
- **잔상 제거**: 마커(안티앨리어싱 번짐 포함)는 주변 캐릭터 색으로
  inpaint되어 팔레트 오염·눈 잔상이 없다. 눈 rect 안에 어두운 도트가 남는
  종(황룡 눈두덩 등)은 연결된 어두운 덩어리를 `eye_clear`로 자동 산출한다
  (삼족오처럼 얼굴 전체가 어두운 종은 크기 상한으로 제외).
- **흰 몸통 복구**: 흰 캐릭터(달토끼)가 배경으로 새어 마커가 본체 밖에
  남으면 closing+fill_holes로 아웃라인 틈을 봉합해 얼굴을 복구한다.
- **새 캐릭터 추가 절차**: 스테이지 3마리가 좌→우로 나열된 픽셀아트에
  눈(마젠타)·입(시안) 점을 찍어 `tool/refs/{종}_눈입.png`로 저장 →
  `SPECIES`에 경로 추가 → 덤프 → 모션 재생성. 캐릭터는 왼쪽을 봐야 한다
  (오른쪽을 보면 flip=True).

프로그램 파생 대신 실제 레퍼런스의 실루엣과 5색 팔레트를 보존한다:
...
도깨비·달토끼처럼 양눈이 보이는 스프라이트는 마커 2개가 rect 2개로 잡혀
`joy`/`angry`/`hurt` 표정이 양쪽 눈에 함께 그려진다. 입 마커는 공격
브레스·배고픔 침 효과가 주둥이/부리에서 시작하는 기준점이 된다.
...
**성장기(`'{종}2'` 36)·성숙기(stage 4, 사신수 `'{종}3'`/일반종 `'{종}3n'` 56)는 이 레시피대로 구현 완료.**
성숙기에서 확립된 추가 패턴:

- **원본 덤프 아트**: 56처럼 큰 그리드는 업스케일 없이 원본 PNG를 목표 크기로 바로 덤프해
