...
```jsonc
{
  ...
  "def": 20,
  "hp": 90,
  "todaySteps": 6200            // 회피율 계산용 (아래 배틀 규칙 참조)
}
```
...
- 배틀 시뮬레이션 규칙은 `BattleWithActivityUseCase`와 동일해야 체감 일관성이
  ...
  단방향, **회피 = 기본 5% + 오늘 걸음수/10,000 × 10% (최대 15%)**
  (회피 시 데미지 0·디버프 미적용 — PvP는 양측 각자의 걸음수로 계산하며,
  걸음수를 페이로드로 받으려면 petPayload에 `todaySteps` 필드를 추가할 것),
