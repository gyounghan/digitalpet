...
void main() {
  ...
  group('babyMotionFrames 데이터 무결성', () {
    ...
    test('모든 프레임은 32x32이고 dark/body가 겹치지 않는다', () {
      for (final s in species) {
        for (final m in motions) {
          final frames = babyMotionFrames[s]![m]!;
          for (var i = 0; i < frames.length; i++) {
            final frame = frames[i];
            expect(frame.size, 32, reason: '$s/$m[$i]: size');
            expect(frame.dark.length, 32, reason: '$s/$m[$i]: dark rows');
            expect(frame.body.length, 32, reason: '$s/$m[$i]: body rows');
            for (var y = 0; y < 32; y++) {
